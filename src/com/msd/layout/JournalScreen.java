...
public class JournalScreen extends Application  {
	...
	JournalScreen(String conf){
		...
		pubList = p.getMostRecentPublications(conf,"J");
		...
		pubGrid.setVgap(10);
			...
			for(PublicationMO P:pubList){
				...
				if(P.getName().length()>29)
					{ pubTitle = new Text(P.getName().substring(0, 30)+"...");}
				
				else { pubTitle = new Text(P.getName());}
				
				pubTitle.setWrappingWidth(300);
	...
	List<Filters> getCustomFilters(){
		...
		displayAuthorsList = Filters.buildAuthorsList(displayPubList);
				...
				if("YEAR".equalsIgnoreCase(filters.get(i).getFilterType())){
					...
					apply.setOnMouseClicked(new EventHandler<MouseEvent>() {
							...
							public void handle(MouseEvent event) {
								...
								int year1 = Integer.parseInt(y1.getText().trim());
								int year2 = Integer.parseInt(y2.getText().trim());
